...
TC_CP=${TC_CP}:${WD}/resources:${TC_HOME}/client/logging/impl

echo "Compiling the sample class.."
"$JAVAC" -classpath "$TC_CP" "${WD}/src/EhCache3ClusteredCacheXML.java"

echo "Starting the ehcache3 sample client, it's going to try to connect to your local server.."
"$JAVA" -Xmx200m -classpath "$TC_CP" EhCache3ClusteredCacheXML
